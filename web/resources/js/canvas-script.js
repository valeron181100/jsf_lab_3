...
$(document).ready(()=>{
    ...
    draw();
    displaySliderValue('coordform:hidden-txt-x', 'coordform:output-txt-x');
    displaySliderValue('coordform:hidden-txt-r', 'coordform:output-txt-r');
    redraw();
...
function rSliderMoved(event, ui) {
    let el = document.getElementById('coordform:hidden-txt-r');
...
function redraw() {
    ...
    let jsonSource = document.getElementById("coordform:hitoryJStext");
    let r = getR();
    JSON.parse(jsonSource.innerText).forEach((p, i)=>{
        if(p.r == r){
        if(p.entered){
